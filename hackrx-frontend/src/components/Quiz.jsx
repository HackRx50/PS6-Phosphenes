...
const Quiz = () => {
  ...
  const [showFeedback, setShowFeedback] = useState(false);
  const [isCorrect, setIsCorrect] = useState(false);
  const [showNextButton, setShowNextButton] = useState(false); // New state for "Next Question" button
  ...
  const handleAnswerOptionClick = (selectedOption) => {
    ...
    setShowFeedback(true);
    setShowNextButton(true); // Show "Next Question" button after answering
  };
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < quizData.length - 1) {
      setCurrentQuestion(currentQuestion + 1);
      setShowFeedback(false);
      setShowNextButton(false); // Hide "Next Question" button for the next question
      resetTimer(); 
    } else {
      setShowScore(true);
    }
  ...
  const handleReplay = () => {
    ...
    setShowScore(false);
    setShowFeedback(false);
    setShowNextButton(false); // Reset "Next Question" button on replay
    resetTimer(); 
  };
