...
def generate_audio_from_text(text, output_audio_path, language='hi'):
    try:
...
def speed_up_audio(input_audio_path, output_audio_path, background_music_path, speed=1.2, music_volume=-15):
    try:
...
language_code = language_codes.get(selected_language, 'hi')


# Generate audio from the speech text
generate_audio_from_text(speeches, audio_output_path, language=language_code)
